                      ...
                      failureBlock:(MHAsiFailureBlock)failureBlock
{
    if (self = [super init])
    {
        ...
        manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/xml",@"application/json",@"text/json",@"text/plain",@"text/html", nil];
...
//                K_ALERT(error.localizedDescription);
                if (failureBlock) {
                    [MBProgressHUD showAlert:error.localizedDescription afterDelay:2];
            ...
            NSString *jsonString = params[@"params"];
...
//            [mutStr replaceOccurrencesOfString:@"\n"withString:@""options:NSLiteralSearch range:range2];
            ...
            [manager POST:url parameters:jsonString success:^(NSURLSessionDataTask * _Nonnull task, id responseObject) {
                [MBProgressHUD hideHUDForView:nil animated:YES];
                DTLog(@"\n\n----请求的返回结果 %@\n",responseObject);
                if (successBlock) {
                    successBlock(responseObject);
